...
export default function Contact() {
  ...
  function onSubmit(e) {
    ...
    // Fallback to PROFILE if settings.socials.whatsapp isn't defined
    const phone = settings?.socials?.whatsapp || PROFILE.links.whatsapp || ''
    const cleanPhone = phone.replace(/[^0-9]/g, '')

    // Construct the pre-filled message
    const text = `Hi, I'm ${form.name.trim()}.\n\n${form.message.trim()}`
          ...
          <div className="grid gap-6 lg:grid-cols-3">
                ...
                <form className="grid gap-6 sm:grid-cols-2" onSubmit={onSubmit}>
                  ...
                  <div className="sm:col-span-2 flex flex-wrap items-center gap-4 mt-2">
                    ...
                    >
                      ...
                      {status === 'sending' ? 'Redirecting…' : 'Chat on WhatsApp'}
                    </button>
                  </div>
            ...
            <Reveal delay={0.08}>
              <Surface className="p-8 h-full bg-zinc-50 dark:bg-zinc-900/40 border-zinc-100 dark:border-zinc-800">
                ...
                <p className="mt-2 text-xl font-bold text-zinc-900 dark:text-white">{settings?.socials?.whatsapp || PROFILE.links.whatsapp || '+917780834427'}</p>
